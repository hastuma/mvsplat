...
VISUALIZE_DIR="/project/winston/mvsplat/outputs/visualize"
INITIAL_COL_START=398
INCREMENT=2
NUM_ITERATIONS=50
...
for i in $(seq 1 $NUM_ITERATIONS); do
    # 計算新的 col_start 值
    NEW_COL_START=$((INITIAL_COL_START - (i - 1) * INCREMENT))
    
    echo "=========================================="
    echo "迭代 $i / $NUM_ITERATIONS"
    echo "設定 col_start = $NEW_COL_START"
    echo "=========================================="
    
    # 修改 crop_offsets.json 中的 col_start 值
    # 使用 jq 修改 JSON 文件
    jq ".\"JAX_004_015_RGB\".col_start = $NEW_COL_START" "$CROP_OFFSET_FILE" > "${CROP_OFFSET_FILE}.tmp"
    mv "${CROP_OFFSET_FILE}.tmp" "$CROP_OFFSET_FILE"
    
    echo "已更新 crop_offsets.json，col_start = $NEW_COL_START"
    ...
    if [ -d "$LATEST_DIR" ]; then
        ...
        if [ -f "$SOURCE_IMAGE" ]; then
            DEST_IMAGE="$VISUALIZE_DIR/step_000000_ctx2_iter${i}_col_start_${NEW_COL_START}.png"
...
jq ".\"JAX_004_015_RGB\".col_start = $INITIAL_COL_START" "$CROP_OFFSET_FILE" > "${CROP_OFFSET_FILE}.tmp"
mv "${CROP_OFFSET_FILE}.tmp" "$CROP_OFFSET_FILE"
echo "✅ crop_offsets.json 已恢復，col_start = $INITIAL_COL_START"
